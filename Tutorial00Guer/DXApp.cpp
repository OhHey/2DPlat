...
DXApp::DXApp(HINSTANCE hInstance)
{
	...
	m_ClientHeight = 600;
	m_AppTitle = "2D GAME";
	m_WndStylel = WS_OVERLAPPEDWINDOW;
	g_pApp = this;
...
int DXApp::Run()
{
	...
	while(WM_QUIT != msg.message)
	{
		...
		{
			if(theTimer.UpdateTime()){
				//Check For Input
				theInput.checkInput();
				//Update
				Update(theTimer.frameTime);
				//Render
				Render(0.0f);
			}
		}
...
bool DXApp::InitWindow()
{
	...
	AdjustWindowRect(&r, m_WndStylel, FALSE); //false is menu
	...
	m_hAppWnd = CreateWindow("DXAPPWNDCLASS", m_AppTitle.c_str(), m_WndStylel, x, y, width, height, NULL, NULL, m_hAppInstance, NULL);
	if(!m_hAppWnd)
	{
		OutputDebugString("\nFAILED TO CREATE WINDOW CLASS\n");
		return false;
...
bool DXApp::InitDirect3D()
{
	...
	{
		result = D3D11CreateDeviceAndSwapChain(NULL, driverTypes[i], NULL, createDeviceFlags, featureLevels, numFeatureLevels, D3D11_SDK_VERSION, 
